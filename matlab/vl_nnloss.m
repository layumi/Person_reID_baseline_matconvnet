...
opts.classWeights = [] ;
opts.threshold = 0 ;
opts.rate = 0;  %normal label smooth
opts.loss = 'softmaxlog' ;
opts.gan = 1; %tradeoff between two loss: softmax and labelsmooth
opts.topK = 5 ;
...
switch lower(opts.loss)
    case {'log', 'softmaxlog', 'mhinge', 'mshinge','labelsmooth'}
...
if nargin <= 2 || isempty(dzdy)
    switch lower(opts.loss)
        ...
        case 'labelsmooth'
            ...
            part1 = t1;  %-log(p(x_gt))
            %use psudo distribution rather than 1/K
            part2 = log(sum(ex,3)) - sum(bsxfun(@minus,x,Xmax),3) * 1/K;
            %z = -log(ex./repmat(sum(ex,3),1,1,size(x,3),1)) .* distribution;
            %part2 = sum(z,3);
            %---------return
    ...
    switch lower(opts.loss)
        ...
        case 'labelsmooth'
            ...
            %global distribution;
            %distribution = reshape(distribution,1,1,size(distribution,1),size(distribution,2));
            %part2 = log(sum(ex,3)) - sum(bsxfun(@minus,x,Xmax) .* distribution,3);
